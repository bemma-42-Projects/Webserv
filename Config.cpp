...
// cette fonction prend en paramètre l'URL demandée par le client
// par exemple, "/src/index.html"
// elle renvoie un pointeur vers l'objet Location qui contient les règles à appliquer
Location* Config::matchLocation(std::string requestPath) 
{
	// on initialise bestMatch à NULL
	// le bestMatch sera le plus grand path trouvé (le plus précis)
    Location* bestMatch = NULL;

	// on initialise la taille du path du bestMatch à 0
    size_t longestLen = 0;

	// on prépare un itérateur pour parcourir le tableau
	// qui contiendra toutes les locations
    std::vector<Location>::iterator it;

	// on boucle pour passer en revue chaque location enregistrée dans la config
    for (it = location_.begin(); it != location_.end(); ++it)
    {
		// on récupère le chemin de la location actuelle enregistrée dans la config
        std::string locPath = it->getPath();
        
		// on cherche locPath dans requestPath
		// si find renvoie 0, ca signifie que l'URL demandée commence
		// par le chemin de la location
		// 
        if (requestPath.find(locPath) == 0) 
        {
			// si on a une correspondance, on vérifie si elle est plus précise (plus longue)
			// que celle qu'on a pu trouver avant
            if (locPath.length() > longestLen) 
            {
				// on met à jour la nouvelle longueur
                longestLen = locPath.length();
				// on sauvegarde cette location
                bestMatch = &(*it);
            }
        }
    }
	// on renvoie la location la plus précise
    return bestMatch;
}
